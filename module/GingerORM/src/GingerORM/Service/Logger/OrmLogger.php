<?php
namespace GingerORM\Service\Logger;
...
class OrmLogger implements Job\Run\LoggerInterface
{
    ...
    {
        $jobRunEntity = $this->entityManager->getRepository('GingerORM\Entity\JobRun')->find($jobRunId);
    ...
    public function getJobRuns($jobName, $max = 0, $skip = 0)
    {
        ...
        $jobEntity = $this->entityManager->getRepository('GingerORM\Entity\Job')->findOneByName($jobName);

        $jobRunEntitys = $this->entityManager->getRepository('GingerORM\Entity\JobRun')
    ...
    {
        $query = $this->entityManager->getRepository('GingerORM\Entity\JobRun')->createQueryBuilder('jr');
    ...
    public function logMessage($taskRunId, Job\Run\Message $message)
    {
        $taskRunEntity = $this->entityManager
            ->getRepository('GingerORM\Entity\TaskRun')->find($taskRunId);
    ...
    public function startTaskRun($jobRunId, $taskId, $totalItemCount)
    {
        ...
        $taskEntity = $this->entityManager->getRepository('GingerORM\Entity\Task')->find($taskId);
