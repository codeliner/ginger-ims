...
use GingerORM\Entity\Permission;
...
class PermissionRepository extends EntityRepository implements PermissionsLoaderInterface
{
    ...
    {
        $user = $this->getEntityManager()->getRepository('GingerORM\Entity\User')
            ->find($userId);
        ...
        $job = $this->getEntityManager()->getRepository('GingerORM\Entity\Job')
    ...
    {
        $user = $this->getEntityManager()->getRepository('GingerORM\Entity\User')->find($userId);
        
        if ($user) {
            $this->getEntityManager()
                ->createQuery('DELETE GingerORM\Entity\Permission p WHERE p.user = :user')
    ...
    private function loadPermissionsEntities($userId)
    {
        ...
        $query->select('p')->from('GingerORM\Entity\Permission', 'p')
            ->where('p.user = :user')
            ->setParameter('user', $user);
    ...
    /**
     * 
     * @param \GingerORM\Entity\Permission $permission
     * @return array
     */
